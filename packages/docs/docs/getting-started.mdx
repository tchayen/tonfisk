...
Set up `vanilla-extract` according to your bundler settings: [guide](https://vanilla-extract.style/documentation/setup/). `vanilla-extract` will consume styles from Tonfisk and produce a final bundle using only the ones you use in your code.

> I am planning to provide alternative bundle with all CSS packed together.
>
> Pros: would allow people to use `tonfisk` without setting up `vanilla-extract` (for example in CRA).
>
> Cons: wouldn't allow for tree-shaking the CSS as it would be a final bundled `main.css` file.
...
If you are looking for inspiration, make sure to check [Example](/example) which features advanced usage of popovers and modals. [`examples/`](https://github.com/tchayen/tonfisk/tree/main/examples) shows how to set up Tonfisk with Webpack and Next.js.
